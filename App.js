...
export default function App() {
  ...
  const apriAggiungi = () => {
    setForm({ nome: "", categoria: categorie[0] || "", unita: "Bottiglie", stock: "", minimo: "" });
    setModal({ type: "add" });
  };

  const apriModifica = (p) => {
    setForm({ nome: p.nome, categoria: p.categoria, unita: p.unita, stock: p.stock, minimo: p.minimo });
  ...
  const salva = async () => {
    ...
    if (modal.type === "add") {
      ...
      await supabase.from("products").update(data).eq("id", modal.product.id);
    }
    setModal(null);
  };
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
            ...
            {editBarName ? (
              <input autoFocus value={barName} onChange={e => setBarName(e.target.value)} onBlur={() => setEditBarName(false)} onKeyDown={e => e.key === "Enter" && setEditBarName(false)}
                style={{ ...inputStyle, width: "180px", padding: "4px 8px", fontSize: isMobile ? "16px" : "20px", background: "transparent", border: "none", borderBottom: "1px solid #c9a84c", borderRadius: 0 }} />
            ) : (
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "8px", marginBottom: "14px" }}>
              {categorie.map(cat => (
                <div key={cat} style={{ display: "flex", alignItems: "center", gap: "10px", background: "#0d0d0f", borderRadius: "8px", padding: "10px 14px" }}>
                  <input type="color" value={catColors[cat] || "#888"} onChange={e => aggiornaCatColore(cat, e.target.value)}
                    style={{ width: "32px", height: "32px", border: "none", background: "none", cursor: "pointer", padding: 0, borderRadius: "50%" }} />
                  ...
                  <button onClick={() => eliminaCategoria(cat)} style={{ background: "transparent", border: "none", color: "#ff4444", cursor: "pointer", fontSize: "16px", padding: "0 4px" }}>✕</button>
                </div>
              ))}
            </div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "6px" }}>
              {products.map(p => (
                <div key={p.id} onClick={() => apriModifica(p)} style={{ display: "flex", alignItems: "center", gap: "10px", background: "#0d0d0f", borderRadius: "8px", padding: "10px 14px", cursor: "pointer", borderLeft: `3px solid ${catColors[p.categoria] || "#888"}` }}>
                  ...
                  <span style={{ color: getStockColor(p), fontWeight: "bold", fontSize: "14px" }}>{p.stock}</span>
      ...
      {modal && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.82)", display: "flex", alignItems: isMobile ? "flex-end" : "center", justifyContent: "center", zIndex: 100 }}
          ...
          <div style={{ background: "#161410", border: "1px solid #3a2e1a", borderRadius: isMobile ? "20px 20px 0 0" : "12px", padding: isMobile ? "24px 20px 36px" : "32px", width: isMobile ? "100%" : "min(460px,92vw)" }}>
            ...
            <h2 style={{ margin: "0 0 20px", fontSize: "18px", fontWeight: "normal", color: "#f5ead5" }}>
              {modal.type === "add" ? "➕ Aggiungi prodotto" : "✏️ Modifica prodotto"}
            </h2>
